document.addEventListener('DOMContentLoaded', function () {
  ...
  document.body.addEventListener('submit', function (e) {
    ...
    fields.forEach(function (field) {
      ...
      if (field.hasAttribute('required') && value === '') {
        errorMessage = getErrorMessage('required', name);
      } else if (value !== '') {
        if (type === 'email' && !isValidEmail(value)) {
          errorMessage = getErrorMessage('email', name);
        } else if (type === 'tel' && !isValidPhone(value)) {
          errorMessage = getErrorMessage('tel', name);
        } else if (field.hasAttribute('minlength')) {
          const min = parseInt(field.getAttribute('minlength'), 10);
          if (value.length < min) {
            errorMessage = getErrorMessage('minlength', name, min);
          }
        } else if (field.hasAttribute('maxlength')) {
          const max = parseInt(field.getAttribute('maxlength'), 10);
          if (value.length > max) {
            errorMessage = getErrorMessage('maxlength', name, max);
          }
        }
      }
  ...
  function createSuccessMessage() {
    ...
    el.textContent = 'Your message has been sent successfully. We will get back to you soon.';
    return el;
  }
  ...
  function initScrollAnimations() {
    const elements = document.querySelectorAll(
      ...
      '.testimonial, .pricing-card, .cta-content'
    );
  ...
  // Re-run for dynamically added elements (basic support)
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: 'f8476ab8-2062-4d6b-8ce9-15bc20ec35db',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || 'f8476ab8-2062-4d6b-8ce9-15bc20ec35db';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'f8476ab8-2062-4d6b-8ce9-15bc20ec35db';
